...
export interface DriveResources {
  folderId: string;
...
export interface CampPatient {
  ...
  visionRight?: string;
  visionLeft?: string;
  dryRefractionRight?: string;
  dryRefractionLeft?: string;
  acceptanceRight?: string;
  acceptanceLeft?: string;
  ocularDiagnosis?: string;
  outcome?: string;
  updatedAt: string;
  createdAt: string;
}
...
class GoogleDriveService {
  ...
  private syncQueue: {campId: string, patients: CampPatient[]}[] = [];
  ...
  setCredentials(token: string, expiresIn: number) {
    this.credentials = {
      accessToken: token,
  ...
  async createCampResources(campName: string, campDate: Date, assignedStudents: string[]): Promise<DriveResources | null> {
  ...
  private async createExcelTemplate(parentFolderId: string, campName: string, formattedDate: string): Promise<string | null> {
    try {
      ...
      const headers = [
        'Patient ID', 'Name', 'Age', 'Sex', 'History', 
        'Vision Right', 'Vision Left', 
        'Dry Refraction Right', 'Dry Refraction Left',
        'Acceptance Right', 'Acceptance Left',
        'Ocular Diagnosis', 'Outcome',
  ...
  private async shareWithStudents(fileId: string, studentEmails: string[]) {
    try {
  ...
  async syncPatientData(campId: string, patients: CampPatient[], spreadsheetId: string): Promise<boolean> {
    if (!this.isAuthenticated()) {
  ...
  private queueSyncOperation(campId: string, patients: CampPatient[]) {
    ...
    if (existingIndex !== -1) {
      ...
      this.syncQueue.push({ campId, patients });
    }
